...
from pydclib.pydclib import mnemonic2bin, hstr2bin, bin2hstr

from dc.core import config
from dc.core.Wallet import Wallet, WalletDecryptionError
from dc.core.misc.helper import parse_hexblob, parse_qaddress
from dc.core.MultiSigAddressState import MultiSigAddressState
...
from dc.crypto.xmss import XMSS, hash_functions
from dc.generated import dc_pb2_grpc, dc_pb2

ENV_dc_WALLET_DIR = 'ENV_dc_WALLET_DIR'
...
class CLIContext(object):
    ...
    def get_stub_public_api(self):
        ...
        return dc_pb2_grpc.PublicAPIStub(channel)
...
def _public_get_address_balance(ctx, address):
    ...
    get_address_state_req = dc_pb2.GetAddressStateReq(address=parse_qaddress(address))
...
@click.version_option(version=config.dev.version, prog_name='dc Command Line Interface')
@click.group()
...
@click.option('--wallet_dir', default='.', help='local wallet dir', envvar=ENV_dc_WALLET_DIR)
@click.option('--json', default=False, is_flag=True, help='output in json')
@click.pass_context
def dc(ctx, verbose, host, port_pub, wallet_dir, json):
    """
...
def tx_push(ctx, txblob):
    ...
    stub = ctx.obj.get_stub_public_api()
    pushTransactionReq = dc_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
...
@dc.command()
@click.option('--src', type=str, default='', prompt=True, help='signer dc address')
@click.option('--master', type=str, default='', prompt=True, help='master dc address')
@click.option('--addr_to', type=str, default='', prompt=True, help='dc Address receiving this message (optional)')
...
@dc.command()
@click.option('--src', type=str, default='', prompt=True, help='source dc address')
@click.option('--master', type=str, default='', prompt=True, help='master dc address')
...
@dc.command()
@click.option('--src', type=str, default='', prompt=True, help='signer dc address')
@click.option('--master', type=str, default='', help='master dc address')
...
@click.option('--owner', default='', prompt=True, help='Owner dc address')
...
def tx_token(ctx, src, master, symbol, name, owner, decimals, fee, ots_key_index):
    ...
    while True:
        ...
        initial_balances.append(dc_pb2.AddressAmount(address=parse_qaddress(address),
                                                      amount=amount))
...
def slave_tx_generate(ctx, src, master, number_of_slaves, access_type, fee, pk, ots_key_index):
    ...
    try:
        ...
        click.echo('Move slaves.json file from current directory to the mining node inside ~/.dc/')
    except Exception as e:
...
@dc.command()
@click.option('--owner', default='', prompt=True, help='source dc address')
@click.pass_context
def token_list(ctx, owner):
    ...
    try:
        stub = ctx.obj.get_stub_public_api()
        address_state_req = dc_pb2.GetAddressStateReq(address=owner_address)
        ...
        for token_hash in address_state_resp.state.tokens:
            get_object_req = dc_pb2.GetObjectReq(query=bytes(hstr2bin(token_hash)))
...
def state(ctx):
    ...
    """
    stub = ctx.obj.get_stub_public_api()
    nodeStateResp = stub.GetNodeState(dc_pb2.GetNodeStateReq())
