...
from dc.core.misc import ntp, logger, set_logger
from dc.core.dcnode import dcNode
from dc.services.services import start_services
...
def parse_arguments():
    ...
    parser.add_argument('--miningAddress', dest='mining_address', required=False,
                        help="dc Wallet address on which mining reward has to be credited.")
...
def main():
    ...
    logger.debug("=====================================================================================")
    logger.info("dc Path: %s", args.dc_dir)
    ...
    dcnode = dcNode(mining_address=mining_address)
    dcnode.set_chain_manager(chain_manager)

    set_logger.set_logger(args, dcnode.sync_state)

    #######
    # NOTE: Keep assigned to a variable or might get collected
    admin_service, grpc_service, mining_service, debug_service = start_services(dcnode)

    dcnode.start_listening()

    dcnode.start_pow(args.mining_thread_count)

    logger.info('dc blockchain ledger %s', config.dev.version)
    if config.user.mining_enabled:
