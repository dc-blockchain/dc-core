...
from pydclib.pydclib import bin2hstr, dcHelper, XmssFast
...
from dc.core.txs import build_tx as main_build_tx
...
class Transaction(object, metaclass=ABCMeta):
    ...
    def addr_from(self):
        ...
        return bytes(dcHelper.getAddress(self.PK))
    ...
    @staticmethod
    def from_pbdata(pbdata: dc_pb2.Transaction):
    ...
    @staticmethod
    def get_slave(tx):
        addr_from_pk = bytes(dcHelper.getAddress(tx.PK))
    ...
    def set_affected_address(self, addresses_set: set):
        ...
        addresses_set.add(bytes(dcHelper.getAddress(self.PK)))
    ...
    def validate_all(self, state_container: StateContainer, check_nonce=True) -> bool:
        ...
        addr_from_pk = bytes(dcHelper.getAddress(self.PK))
    ...
    def _coinbase_filter(self):
        if config.dev.coinbase_address in [bytes(dcHelper.getAddress(self.PK)), self.master_addr]:
